...
public:
	...
	int readin_parsescript_configfile( const char* configfilename)
	{
	  	...
	  	while( currentline.ReadLine( configfile ) && !currentline.BeginsWith("endlist") )
	    {
	    	if( !currentline.BeginsWith("#") )
	    	{
		    	...
		    	if ( ntokens == 1 )
		    	{
		    		TString sval = ( (TObjString*)(*tokens)[0] )->GetString();

		    		int thisRunNum = sval.Atoi();

		    		m_runlist_vec.push_back(thisRunNum);

		    		std::cout << "FOUND RUN" << thisRunNum << std::endl;
		    	}
	  		...
	  		if ( !currentline.BeginsWith("#") )
	  		{
		    	...
		    	{	
		      		...
		      		else if( skey == "output_filename" )
		      		{
						TString sval = ( (TObjString*)(*tokens)[1] )->GetString();
						m_outfilename = sval;
						std::cout << "Output file name: " << m_outfilename << endl;
		      		}
		    	...
		    	else if ( ntokens==1 )
		    	{
		    		...
		    		if( skey == "pass" )
		      		...
		      		else if( skey == "input_dir" )
		      		{
						...
						std::cout << "Warning: Input mass replayed ROOT file dir not provided. Using default dir: " << m_input_dir <<'\n';
					}
	...
	std::vector<int> return_RunListVector()
	{
		return m_runlist_vec;
	}
